...
// on a different port (6088, the default for HTTP in Janus), which is
// why we make use of the 'window.location.hostname' base address. Since
// Janus can also do HTTPS, and considering we don't really want to make
// use of HTTP for Janus if your demos are served on HTTPS, we also rely
// on the 'window.location.protocol' prefix to build the variable, in
// particular to also change the port used to contact Janus (6088 for
// HTTP and 6089 for HTTPS, if enabled).
...
//
// 		var server = "ws://" + window.location.hostname + ":6188";
//
...
//
//		var server = [
//			"ws://" + window.location.hostname + ":6188",
//			"/janus"
//		];
//
...
if(window.location.protocol === 'http:')
	server = "http://" + window.location.hostname + ":6088/janus";
else
	server = "https://" + window.location.hostname + ":6089/janus";
									...
									echotest.createOffer(
										{
											...
											customizeSdp(jsep) {
												// Offer multiopus
												jsep.sdp = jsep.sdp
													.replace("opus/46000/2", "multiopus/46000/6")
